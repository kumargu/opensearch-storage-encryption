...
public final class EgarDecryptedCryptoMMapDirectory extends MMapDirectory {

    private static final Logger LOGGER = LogManager.getLogger(LazyDecryptedCryptoMMapDirectory.class);

    private final KeyIvResolver keyIvResolver;

    public EgarDecryptedCryptoMMapDirectory(Path path, Provider provider, KeyIvResolver keyIvResolver) throws IOException {
        super(path);
    ...
    public void decryptSegmentInPlaceParallel(Arena arena, MemorySegment segment, long segmentOffsetInFile) throws Throwable {
        ...
        if (size <= (4L << 20)) {
            ...
            OpenSslNativeCipher.decryptInPlace(segment.address(), size, key, iv, segmentOffsetInFile);
